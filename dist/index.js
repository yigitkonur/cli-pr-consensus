...
async function run(url, options) {
    ...
    try {
        ...
        if (!options.quiet) {
            spinner?.start(chalk.blue(`Fetching PR #${pr.number} from ${pr.owner}/${pr.repo}...`));
        }
        // Collect data
        const data = await collectPRData(url, options);
        if (!options.quiet) {
            spinner?.succeed(chalk.green(`Fetched PR #${pr.number}: ${data.pr.title}`));
        }
        // Format output
        let output;
        ...
        if (options.verbose && !options.quiet) {
            ...
            console.error(chalk.dim(`  Comments: ${data.comments.general.length + Object.values(data.comments.byFile).flat().length}`));
