 ...
 */
export function formatAsConsensus(data: PRConsensusOutput): string {
 ...
 */
function buildConsensusStructure(data: PRConsensusOutput): ConsensusOutput {
...
): Map<string, ParsedComment[]> {
  const groups = new Map<string, ParsedComment[]>();
...
function buildFileSections(
  fileGroups: Map<string, ParsedComment[]>,
  files: PRConsensusOutput['files']
): ConsensusFileSection[] {
  const sections: ConsensusFileSection[] = [];
 ...
 */
function groupCommentsByLine(comments: ParsedComment[]): ConsensusLineGroup[] {
  // Sort by line number
  ...
  const groups: ConsensusLineGroup[] = [];
  let currentGroup: ConsensusLineGroup | null = null;
 ...
 */
function toConsensusComment(comment: ParsedComment): ConsensusComment {
  return {
    agent: comment.agent,
 ...
 */
function renderConsensusMarkdown(consensus: ConsensusOutput): string {
  const lines: string[] = [];
